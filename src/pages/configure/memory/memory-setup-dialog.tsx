...
interface MemorySetupDialogProps {
  provider: IMemoryProvider;
}

type ViewMode = "list" | "add";

export const MemorySetupDialog: React.FC<MemorySetupDialogProps> = ({
  provider,
}) => {
  ...
  const handleSubmitCredential = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      // Register role ARN with Lambda (same as Bedrock LLM)
      if (
        ...
        try {
          ...
          console.log(
            `Lambda called successfully with role_arn ${credentialsObj.aws_role_name}`,
          );
        } catch (error) {
      ...
      rhfForm.reset();
      queryClient.invalidateQueries({ queryKey: ["getMemoryCredentials"] });
      setViewMode("list");
    } catch (error: any) {
      toast({
        title: "Error",
              ...
              <span className="inline-flex items-center space-x-1">
                ...
                {provider.meta_data.lambda_registration_required && (
                  <>
                    ...
                    <Link
                      download
                      to="/lyzr-aws-agentcore-iam.yaml"
                      target="_blank"
                      className="text-link underline-offset-4 hover:underline"
                    >
              ...
              {isFetchingCredentials ? (
                ...
                <div className="flex flex-col items-center justify-center py-8">
                  <p className="text-muted-foreground">
                    No memory credentials configured
                  </p>
                  <Button
                    variant="link"
                    onClick={() => setViewMode("add")}
                  ...
                  {/* <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} placeholder="My AWS Memory" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
