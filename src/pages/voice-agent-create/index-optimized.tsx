...
export default function CreateAgent() {
      ...
      if (response.data) {
        ...
        if (isStringifiedJSONString(agent.tool_usage_description)) {
          ...
          toolsData = agent.tools.map((toolName, index) => ({
            ...
            persist_auth: agent.tool_configs?.[index]?.persist_auth,
        ...
        if (agent.tool_configs && agent.tool_configs.length > 0) {
          toolsData = agent.tool_configs.flatMap((tool: any) =>
            tool.action_names.map((action: any) => ({
              ...
              persist_auth: tool.persist_auth,
        ...
        const initialValues = {
          ...
          store_messages: agent.hasOwnProperty("store_messages")
            ...
            : true,
          language: agent?.voice_config?.language || "English",
          ai_intro_text: agent?.voice_config?.message || "",
          who_speaks_first: agent?.voice_config?.initiator || "human",
          voice: agent?.voice_config?.voice || "sage",
        };
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const concatenatedToolDescription = validTools
        ...
        .join("\n");
      // `managed_agents` should be an array (per `schema.ts`), but we tolerate
      // any stale legacy shapes to avoid runtime submit errors.
      const managedAgentsList = Array.isArray(values.managed_agents)
        ? values.managed_agents
        : // @ts-expect-error - tolerate legacy shape
          Array.isArray(values.managed_agents?.agents)
          ? // @ts-expect-error - tolerate legacy shape
            values.managed_agents.agents
          : [];

      const managedAgentsData = managedAgentsList.map((agent: any) => ({
        id: agent.id,
        name: agent.name || "",
        usage_description: agent.usage_description,
      }));

      const hasFileOutput = values.features.some(
        (feature) => feature.type === "FILEASOUTPUT",
      );
      ...
      payload = {
        ...
        voice_config: {
          initiator: values?.who_speaks_first || "human",
          message: values?.ai_intro_text || "",
          language: values?.language || "English",
          voice: values?.voice || "sage",
        },
      };
      // }
      ...
      if (agentId) {
        ...
        setTimeout(
          () =>
            navigate(`/voice-agent-create/${newAgentId}`, {
              replace: true,
            }),
          100,
        );
  ...
  const handleBackClick = () =>
    group_name
      ? navigate(`${Path.VOICE_AGENT_BUILDER}?group_name=${group_name}`)
      : navigate(-1);
                ...
                <>
                  {agentId && (
                    <>
                      ...
                      >
                        <Link
                          to={`/traces?tab=traces&agent_id=${agentId}`}
