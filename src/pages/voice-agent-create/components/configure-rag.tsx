...
export const ConfigureRag: React.FC<ConfigureRagProps> = ({
  ...
  const existingRagId = existingRagConfig?.config?.lyzr_rag?.rag_id;
  const existingTopK = existingRagConfig?.config?.lyzr_rag?.params?.top_k || 5;
  const existingRetrievalType =
    existingRagConfig?.config?.lyzr_rag?.params?.retrieval_type || "basic";
  const existingScoreThreshold =
    existingRagConfig?.config?.lyzr_rag?.params?.score_threshold || 0;
  ...
  const handleOpenChange = (isOpen: boolean) => {
    ...
    if (
      ...
      Object.values(existingRagConfig?.config?.agentic_rag ?? {}).length === 0
    ) {
  ...
  useEffect(() => {
    if (open) {
      if (Object.values(existingRagConfig?.config?.lyzr_rag ?? {}).length > 0) {
        setRagType("basic-rag");
        form.reset({
          rag_id: existingRagConfig?.config?.lyzr_rag?.rag_id,
          retrieval_type:
            existingRagConfig?.config?.lyzr_rag?.params?.retrieval_type ||
            "basic",
          score_threshold:
            existingRagConfig?.config?.lyzr_rag?.params?.score_threshold || 0,
          top_k: existingRagConfig?.config?.lyzr_rag?.params?.top_k || 5,
        });
      } else if (
        Object.values(existingRagConfig?.config?.agentic_rag ?? {}).length > 0
      ) {
        ...
        agenticRagForm.reset({
          agentic_rag: existingRagConfig?.config?.agentic_rag,
        });
      } else {
        setRagType("basic-rag");
      }
