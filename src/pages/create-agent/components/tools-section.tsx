...
const ToolAuthenticationRadioGroup = ({
  ...
  const effectiveValue = value ?? true;
  return (
...
const ToolsSection: React.FC<ToolsSectionProps> = ({ form, agent }) => {
  ...
  const toolSource =
    selectedToolData?.tool_source ??
    editingToolFormData?.tool_source ??
    "composio";
  const toolAppId = selectedToolData?.app_id ?? "";
  const toolServerId =
    selectedToolData?.server_id ?? editingToolFormData?.server_id ?? "";
  const isMcpSelected = (toolSource || "") === "mcp";
  const providerUuid =
    selectedToolData?.provider_uuid ?? editingToolFormData?.provider_uuid ?? "";
  const credentialId =
    selectedToolData?.credential_id ?? editingToolFormData?.credential_id ?? "";
  ...
  const {
    ...
    !isMcpSelected ? (selectedTool ?? undefined) : undefined,
  ...
  useEffect(() => {
    ...
    fetchMcpActions();
  }, [apiKey, isMcpSelected, selectedToolData?.server_id]);
  ...
  const fetchTools = useCallback(async () => {
    try {
      ...
      if (
        ...
        const toolIdsFromUserTools = new Set(
          Array.isArray(res.data)
            ? res.data.map((tool: any) => tool.tool_id).filter(Boolean)
            : [],
        );
        ...
        const filterUserAccounts = Array.isArray(userAccounts?.data)
          ? userAccounts?.data
              .filter(
                (account: any) =>
                  account.provider_uuid !== null &&
                  !toolIdsFromUserTools.has(account.provider_id),
              )
              .map((account: any) => ({
                ...account,
      ...
      {tools.map((field, index) => {
        ...
        return (
          ...
          >
            ...
            <div>
              {isSharedAgent &&
              form.watch(`tools.${index}.persist_auth`) === false ? (
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <TriangleAlert
                        className="h-4 w-4 cursor-pointer text-eval-yellow"
                        onClick={() => {
                    ...
                    <TooltipContent>
                      <p>You need to configure this tool</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ) : (
