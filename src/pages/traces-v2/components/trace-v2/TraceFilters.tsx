...
import { Filter, X, CalendarIcon, AlertCircle, Search, Loader2 } from 'lucide-react';
...
import { TraceFilters as TraceFiltersType } from '../../trace.service';
...
interface TraceFiltersProps {
    ...
    onApplyFilters: (filters: TraceFiltersType) => void;
    onClearFilters: () => void;
    orgMembers: any[];
}

const TraceFiltersComponent: React.FC<TraceFiltersProps> = ({ filters, onApplyFilters, onClearFilters, orgMembers }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [tempFilters, setTempFilters] = useState<TraceFiltersType>(filters);

    // Minimum date for traces-v2 - February 3, 2026
    ...
    // Only sync tempFilters when the applied filters change from parent (not when sheet opens/closes)
    // This preserves unapplied changes when the sheet is closed and reopened
    useEffect(() => {
    ...
    useEffect(() => {
        // Non-admin users should only see their own traces
    ...
    const handleApply = () => {
        ...
        if (tempFilters.start_time && tempFilters.end_time) {
            ...
            if (daysDiff > 31) {
                return; // Don't apply if range exceeds 31 days
            }
        }
    ...
    const handleClear = () => {
        // Non-admin users must keep query_user_id, admin users can clear everything
    ...
    // Count active filters (handle both strings and other values)
    ...
    return (
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent>
                <SheetHeader>
                    <SheetTitle>Filter Traces</SheetTitle>
                    <SheetDescription>
                        Apply filters to narrow down your trace results
                    </SheetDescription>
                </SheetHeader>
                ...
                <div className="space-y-4 py-6">
                                        ...
                                        <Calendar
                                            ...
                                            disabled={(date) => {
                                                ...
                                                if (tempFilters.start_time) {
                                                    ...
                                                    if (differenceInDays(date, startDate) > 31) return true;
                                                }
                                                return false;
                                            }}
                                            initialFocus
                                ...
                                if (daysDiff > 31) {
                                    return (
                                        <div className="flex items-center gap-1.5 text-destructive">
                                            ...
                                            <span className="text-xs">Maximum range is 31 days</span>
                                        </div>
                                    );
                                }
                                return (
                                    <p className="text-xs text-muted-foreground">
                                        Selected range: {daysDiff} day{daysDiff !== 1 ? 's' : ''}
                                    </p>
                                );
                            })()
                        )}
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Search and filter traces by agent name
                        </p>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            value={tempFilters.trace_id || ''}
                            onChange={(e) => setTempFilters((prev) => ({ ...prev, trace_id: e.target.value }))}
                        />
                        <p className="text-xs text-muted-foreground">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(e) => setTempFilters((prev) => ({ ...prev, session_id: e.target.value }))}
                        />
                        <p className="text-xs text-muted-foreground">
...
export default TraceFiltersComponent;
