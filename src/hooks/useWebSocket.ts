...
import { AgentConfig, ChatEvent } from "@/types/chat";
...
export const useWebSocket = (
  ...
  const connectWebSocket = (sessionId: string) => {
    ...
    const wsUrl = `${currentAgent.wsBaseUrl}/ws/${sessionId}?x-api-key=${apiKey}`;
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (event.data.includes("[ERROR]") && event.data.includes("cannot access local variable")) {
          ...
          if (eventData.thinking) {
            ...
            const thinkingEvent = new CustomEvent("thinking_event", {
              detail: {
                ...
                tool_name: eventData.tool_name || eventData.agent_name || eventData.arguments?.name || null,
          ...
          switch (eventData.event_type) {
            ...
            case "tool_call_prepare":
              ...
              handleEvent(
                "tool_call_prepare",
                `${currentAgent.name} asked ${agentName}: ${message}`,
                false,
              );
              break;
            case "tool_response":
